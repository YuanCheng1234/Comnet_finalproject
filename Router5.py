...
if __name__ == '__main__':
    ...
    neighbor_list = get_neighbor(g, "192.168.1.2")
    
    #Create the hello packet to its neighbors 
    Hello_packet = create_hello(1,1,104)
    ...
    #Receive the hello packets from others and create the initial routing table 
    while counter < len(neighbor_list):
        Received_packet = receive_router('192.168.1.2', 1)
    ...
    #Create and send LS packet to its neighbors 
    LS_Packet = create_LSpacket(4, 1, 104, 'DATA')
    ...
    #Planning to receive the LS packets and update the routing table 
    #Plz use a class to contain all the functions into one router class and try to put the while loop (receiving function) into a specific function 
